...
| Foundry agent setup fails with **`cannot import name 'KnowledgeBase'`** (or another `Knowledge*`/`SearchIndex*` model) | A drifted `azure-search-documents` version — the Foundry IQ agentic-retrieval preview models live **only** in `11.7.0b2` | Pinned to `azure-search-documents==11.7.0b2` in `pyproject.toml` and `src/requirements.txt`. If you edited those, restore the exact pin (stable `12.0.0` and beta `12.1.0b1` renamed/removed the classes) and re-run `azd provision`. |
